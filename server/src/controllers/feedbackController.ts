...
export async function saveFeedbackHandler(
	req: Request,
	res: Response,
): Promise<void> {
	...
	if (
		!actualResult ||
		!VALID_RESULTS.includes(actualResult as FeedbackResult)
	) {
		res.status(400).json({
			error: `actualResult는 ${VALID_RESULTS.join(", ")} 중 하나여야 합니다.`,
		});
		return;
	}
	...
	try {
		await Feedback.create({
			...
			estimatedHours,
			actualResult: actualResult as FeedbackResult,
		});
